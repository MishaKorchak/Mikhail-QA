...
from pycodestyle import break_after_binary_operator
...
find_car = [(name, details,) for name, details in car_data.items()
            if details[1] >= search_criteria[0]
            and details[2] >= search_criteria[1]
            and details[4] <= search_criteria[2]

             ]
soorted = sorted(find_car, key=lambda x: x[1][2])[:5]
for car in soorted:
    print(car)
